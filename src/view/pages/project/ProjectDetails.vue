	...
	<v-app>
		<div class="container-fluid my-10" v-if="isLoaded">
					...
					<table class="table">
						<tr class="pb-3">
							<td class="pl-0" style="border-top: none" width="15%">
								<b-img :src="project.companyInfo.photo" fluid alt="Fluid image"></b-img>
							</td>
			...
			<div class="row">
				...
				>
					<div class="flex-grow-1 py-6 px-6" style="background-color: #1D31C6; color: white">
				...
				<v-expansion-panels v-model="suplierDetailsPanel">
					<v-expansion-panel key="0">
						<v-expansion-panel-header color="#F2F3FF">
							<p class="text-primary mb-0 font-weight-boldest ls-2 text-left">SUPPLIER DETAILS</p>
						</v-expansion-panel-header>
						<v-expansion-panel-content>
							<div class="row pt-5">
								<div class="col-2">
									<div class="d-flex justify-content-center">
										<b-img height="100%" width="80%" :src="project.companyInfo.photo" fluid alt="Fluid image"></b-img>
									</div>
								</div>
								<div class="col-10">
			...
			<div class="row mt-5">
				<v-expansion-panels v-model="projectDetailsPanel">
					<v-expansion-panel>
						<v-expansion-panel-header color="#F2F3FF">
							<p class="text-primary mb-0 font-weight-boldest ls-2 text-left">PROJECT FACTSHEET</p>
						</v-expansion-panel-header>
						<v-expansion-panel-content>
							<div class="row my-5">
								<div class="col-12">
									<p class="text-primary font-weight-boldest">{{ project.companyInfo.company_name || "N/A" }}</p>
								</div>
							</div>
							<div class="row mb-5">
								<div class="col-12 d-flex flex-row justify-content-between align-items-end">
									<div class="flex-grow-1">
										...
										<div>
											<p class="font-weight-boldest pb-5" style="border-bottom: 1px solid #F2F3FF">DESCRIPTION</p>
									...
									<div class="flex-grow-1">
										...
										<div>
											<p class="font-weight-boldest pb-5" style="border-bottom: 1px solid #F2F3FF">INVOICE NO.</p>
											<p>{{ project.invoice.invoice_no }}</p>
										</div>
									</div>
									<div class="flex-grow-1">
										<div>
											<p class="font-weight-boldest pb-5" style="border-bottom: 1px solid #F2F3FF">INVOICE DATE</p>
											<p>{{ project.invoice.due_date }}</p>
											<p></p>
										</div>
									</div>
							...
							<div class="row px-5 py-8 mb-5 d-flex flex-column" style="background-color: #FAFAFA">
								<div class="d-flex align-items-center mb-5">
									<p class="text-primary font-weight-boldest ls-2 mb-0 mr-5">VERIFIED ON BLOCKCHAIN</p>
									<a
                    ...
                    "
							...
							<div class="activities d-flex justify-content-between flex-row">
								<div
									...
									:key="index"
								>
									<p class="text-primary font-weight-boldest">{{ index + 1 }}. {{ activity.title }}</p>
							...
							<div class="col-10">
								...
								<v-row>
									...
									<v-col cols="5" class="d-flex align-items-center">
										<v-text-field label style="font-size: 13px" v-model.number="amount" placeholder prefix="$"></v-text-field>
										<button style="padding: 0.25rem 0.5rem; max-height: 2rem" class="btn btn-sm btn-info" @click="setMax">Max</button>
									</v-col>
								</v-row>
								...
								>
									...
									>Expected Returns</p>
									<p
										class="ml-4 py-3 my-0 font-weight-boldest text-primary"
									>USD {{ project.loan.interest | money_format }}</p>
								</div>
			...
			<v-dialog v-model="showSuccessfulDialog" persistent max-width="500">
				<v-card class="py-5">
					<div class="d-flex justify-content-center flex-column align-items-center">
						...
						>Funding Successful</p>
						<p class="modal-text" style="color: #1D8CC6">You have successfully funded the invoice.</p>
						<inline-svg width="40%" src="media/pilons/pilon_funding_successful.svg"></inline-svg>
						<v-btn
							style="text-transform: none"
							color="primary"
							squared
...
export default {
	...
	computed: {
    isDisable() {
      return this.amount ? this.amount > this.project.invoice.total_amount : true;
    },
		...mapState({
