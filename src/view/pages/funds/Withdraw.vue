	...
	<div class="container-fluid my-10">
			...
			<div class="col-6">
				<div class="form-group mb-1">
					...
					<div class="input-amount">
						<input type="number" placeholder="0.00" v-model="amount" @input="calculateFee" />
					</div>
				</div>
		...
		<div class="row justify-content-between">
			<div class="col-6">
				...
				<p class="font-weight-boldest">Bank Account no. {{ businessInfo.bank.account_number }}</p>
				<v-btn :loading="isSubmiting" :disabled="!!!amount" color="primary" block large @click="submitFundRequest">SUBMIT</v-btn>
			</div>
			<div class="col-4">
				<inline-svg class="w-100" src="media/pilons/pilon_withdrawal.svg"></inline-svg>
			</div>
		</div>
		...
		<v-dialog v-model="showSuccessDialog" persistent max-width="500">
			<v-card class="py-5">
				<div class="d-flex justify-content-center flex-column align-items-center">
					<p  style="font-size: 1.25rem" class="mt-5 modal-text font-weight-boldest">Withdrawal Successful</p>
					<p class="modal-text">Funds will be debited to your account between 3-5 working days</p>
					<inline-svg width="50%" src="media/pilons/pilon_widrawal successful.svg"></inline-svg>
					<v-btn style="text-transform: none" color="primary" squared @click="$router.push('/funds/overview')">View Funds</v-btn>
				</div>
			</v-card>
		</v-dialog>
	</div>
